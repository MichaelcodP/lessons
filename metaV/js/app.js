(() => {
    ...
    da.init();
    let videoAll = document.querySelectorAll(".video-player");
    let playAll = document.querySelectorAll(".activity__button");
    function toggleVideoStatus(video, index) {
        videoAll.forEach(((element, i) => {
            if (i !== index) {
                element.pause();
                playAll[i].classList.remove("_icon-pause", "_icon-play");
                playAll[i].classList.add("_icon-play");
            }
        }));
        if (video.paused) {
            video.play();
            playAll[index].classList.remove("_icon-play");
            playAll[index].classList.add("_icon-pause");
        } else {
            video.pause();
            playAll[index].classList.remove("_icon-pause");
            playAll[index].classList.add("_icon-play");
        }
    }
    function handleHover(index, isHovered) {
        if (!videoAll[index].paused) return;
        playAll[index].style.opacity = isHovered ? 1 : 0;
    }
    videoAll.forEach(((video, index) => {
        let play = document.querySelectorAll(".play");
        play[index].addEventListener("click", (() => {
            toggleVideoStatus(video, index);
        }));
        [ video, play[index] ].forEach((element => {
            element.addEventListener("mouseenter", (() => handleHover(index, true)));
            element.addEventListener("mouseleave", (() => handleHover(index, false)));
        }));
    }));
    window["FLS"] = true;
    isWebp();
